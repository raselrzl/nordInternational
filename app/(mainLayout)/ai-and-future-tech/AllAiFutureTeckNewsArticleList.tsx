...
async function getAllAiFutureTeckNewsArticleList(
  page: number = 1,
  pageSize: number = 8
  ...
  const [data, totalCount] = await Promise.all([
    prisma.newsArticle.findMany({
      where: { newsCategory: "AI_AND_FUTURE_TECH" },
      take: pageSize,
      skip,
      select: {
        id: true,
    ...
    prisma.newsArticle.count({ where: { newsCategory: "AI_AND_FUTURE_TECH" } }),
  ]);
...
export default async function AllAiFutureTeckNewsArticleList({
  currentPage,
}: {
  currentPage: number;
}) {
  const { articles, totalPages } = await getAllAiFutureTeckNewsArticleList(currentPage);
