...
async function getAllEditorialNewsArticleList(
  page: number = 1,
  pageSize: number = 8
  ...
  const [data, totalCount] = await Promise.all([
    ...
    prisma.newsArticle.count({ where: { newsCategory: "EDITORIAL" } }),
  ]);
...
export default async function AllEditorialNewsArticleList({
  currentPage,
}: {
  currentPage: number;
}) {
  const { articles, totalPages } = await getAllEditorialNewsArticleList(currentPage);
