...
async function getAllFinanceNewsArticleList(
  page: number = 1,
  pageSize: number = 8
  ...
  const [data, totalCount] = await Promise.all([
    ...
    prisma.newsArticle.count({ where: { newsCategory: "FINANCE" } }),
  ]);
...
export default async function AllFinanceNewsArticleList({
  currentPage,
}: {
  currentPage: number;
}) {
  const { articles, totalPages } = await getAllFinanceNewsArticleList(currentPage);
