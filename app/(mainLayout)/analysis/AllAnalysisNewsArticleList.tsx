...
async function getAllAnalysisNewsArticleList(
  page: number = 1,
  pageSize: number = 8
  ...
  const [data, totalCount] = await Promise.all([
    ...
    prisma.newsArticle.count({ where: { newsCategory: "ANALYSIS" } }),
  ]);
...
export default async function AllAnalysisNewsArticleList({
  currentPage,
}: {
  currentPage: number;
}) {
  const { articles, totalPages } = await getAllAnalysisNewsArticleList(currentPage);
