...
async function getAllBooksArtsNewsArticleList(
  page: number = 1,
  pageSize: number = 8
  ...
  const [data, totalCount] = await Promise.all([
    prisma.newsArticle.findMany({
      where: { newsCategory: "BOOKS_AND_ARTS" },
      take: pageSize,
      skip,
      select: {
        id: true,
    ...
    prisma.newsArticle.count({ where: { newsCategory: "BOOKS_AND_ARTS" } }),
  ]);
...
export default async function AllBooksArtsNewsArticleList({
  currentPage,
}: {
  currentPage: number;
}) {
  const { articles, totalPages } = await getAllBooksArtsNewsArticleList(currentPage);
