...
async function getAllBusinessNewsArticleList(
  page: number = 1,
  pageSize: number = 8
  ...
  const [data, totalCount] = await Promise.all([
    ...
    prisma.newsArticle.count({ where: { newsCategory: "BUSINESS" } }),
  ]);
...
export default async function AllBusinessNewsArticleList({
  currentPage,
}: {
  currentPage: number;
}) {
  const { articles, totalPages } = await getAllBusinessNewsArticleList(currentPage);
