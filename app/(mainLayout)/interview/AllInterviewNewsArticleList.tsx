...
async function getAllInterviewNewsArticleList(
  page: number = 1,
  pageSize: number = 8
  ...
  const [data, totalCount] = await Promise.all([
    ...
    prisma.newsArticle.count({ where: { newsCategory: "INTERVIEW" } }),
  ]);
...
export default async function AllInterviewNewsArticleList({
  currentPage,
}: {
  currentPage: number;
}) {
  const { articles, totalPages } = await getAllInterviewNewsArticleList(currentPage);
