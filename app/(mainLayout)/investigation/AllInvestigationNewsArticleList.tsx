...
async function getAllInvestigationNewsArticleList(
  page: number = 1,
  pageSize: number = 8
  ...
  const [data, totalCount] = await Promise.all([
    prisma.newsArticle.findMany({
      where: { newsCategory: "INVESTIGATION" },
      take: pageSize,
      skip,
      select: {
        id: true,
    ...
    prisma.newsArticle.count({ where: { newsCategory: "INVESTIGATION" } }),
  ]);
...
export default async function AllInvestigationNewsArticleList({
  currentPage,
}: {
  currentPage: number;
}) {
  const { articles, totalPages } = await getAllInvestigationNewsArticleList(currentPage);
