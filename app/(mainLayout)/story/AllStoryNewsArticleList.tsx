...
async function getAllStoryNewsArticleList(
  page: number = 1,
  pageSize: number = 8
  ...
  const [data, totalCount] = await Promise.all([
    ...
    prisma.newsArticle.count({ where: { newsCategory: "STORY" } }),
  ]);
...
export default async function AllStoryNewsArticleList({
  currentPage,
}: {
  currentPage: number;
}) {
  const { articles, totalPages } = await getAllStoryNewsArticleList(currentPage);
