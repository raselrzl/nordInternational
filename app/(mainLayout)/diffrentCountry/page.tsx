...
// Map input string to exact Prisma enum
function mapToCountryEnum(input: string): Country | undefined {
  const found = (Object.values(Country) as string[]).find(
    (c) => c.toLowerCase() === input.toLowerCase()
  );
  return found as Country | undefined;
}
...
async function getCountryNews(country: string) {
  ...
  if (!dbCountry) return { allArticles: [], lastFeaturedArticle: null };
              ...
              <div className="grid grid-cols-5">
                ...
                <div className="pl-1 md:pl-4 col-span-5 md:col-span-2">
                  ...
                  {isJson(lastFeaturedArticle.newsDetails) ? (
                    ...
                    <p>{lastFeaturedArticle.newsDetails}</p>
                  )}
                </div>
              </div>
            </Link>
