// app/(mainLayout)/diffrentCountry/[country]/page.tsx
...
export default async function CountryNews({ params }: CountryPageProps) {
          ...
          <div className="hidden md:block pb-2 mb-6 overflow-x-auto pt-4">
            <div className="flex flex-wrap gap-3 px-2">
              {euCountries.map((c) => (
                <Link
                  key={c.name}
                  href={`/diffrentCountry/${encodeURIComponent(c.name)}`}
        ...
        <div className="col-span-5 md:col-span-3">
          ...
          <div className="flex items-center gap-2 pl-2 mb-2">
            <Image
              ...
              className="rounded-sm"
            />
            <h1 className="font-extrabold">{activeCountry?.name || country} Latest</h1>
          </div>

          {/* Featured Article */}
          {lastFeaturedArticle ? (
            <Link href={`/newsDetails/${lastFeaturedArticle.id}`}>
              <div className="grid grid-cols-5 mb-6 max-h-[320px] md:border-1 md:p-2">
                <div className="col-span-5 md:col-span-3 w-full max-h-[270px] md:rounded-xl overflow-hidden mt-10 md:mt-0 border">
                  <Image
                ...
                <div className="col-span-5 md:col-span-2 pl-1 md:pl-4">
                  ...
                  {isJson(lastFeaturedArticle.newsDetails) ? (
                    <div className="text-sm md:text-lg text-accent-foreground/80 mb-2 md:mt-2 line-clamp-3 pl-2">
                      <JsonToHtml json={JSON.parse(lastFeaturedArticle.newsDetails)} />
                    </div>
                  ) : (
                    <p className="text-sm md:text-lg text-accent-foreground/80 mb-2 md:mt-2 line-clamp-3 pl-2">
