...
async function getAllHumanRightsNewsArticleList(
  page: number = 1,
  pageSize: number = 8
  ...
  const [data, totalCount] = await Promise.all([
    prisma.newsArticle.findMany({
      where: { newsCategory: "HUMANRIGHTS" },
      take: pageSize,
      skip,
      select: {
        id: true,
    ...
    prisma.newsArticle.count({ where: { newsCategory: "HUMANRIGHTS" } }),
  ]);
...
export default async function AllHumanRightsNewsArticleList({
  currentPage,
}: {
  currentPage: number;
}) {
  const { articles, totalPages } = await getAllHumanRightsNewsArticleList(currentPage);
